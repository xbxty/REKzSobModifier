...
//备份文件
void BackUpFile(string backupFilePath, string Path_of_File) {  
	ifstream backup_file(backupFilePath);
	if (backup_file.is_open()) {
		...
		if (source_file.is_open()) {
			...
			// 如果打开失败，就输出错误信息
			cout << "Fail to back up! You'd better back up manually!" << endl;
			cout << endl;
		}
		newbackup_file.close();
	}
}
...
int main() {
	...
	//获取文件路径，使用酱圣遗产
	string LocalAppdata = GetLocalAppdata();
	string Path_of_File = LocalAppdata + "\\Katana_ZERO\\KatanaSpeedrunOpt" + to_string(id) + ".zero";
